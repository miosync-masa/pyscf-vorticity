...
def extract_gamma(n_electrons, alphas, method='log'):
    """
    Extract correlation dimension gamma from alpha scaling

    alpha ~ N^(-gamma) -> log(alpha) = -gamma * log(N) + const
    ...
    a_optimal : float
        Optimal exact exchange mixing: a = 1/(1+gamma)
    """
    ...
    # Linear fit: log(alpha) = -gamma * log(N) + const
...
def compute_alpha(E_corr, V):
    """
    Compute coupling constant alpha = |E_corr| / V
