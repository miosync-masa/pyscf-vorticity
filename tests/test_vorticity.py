...
def test_h2_dissociation_trend():
    """Test that alpha shows U-shape during dissociation"""
    alphas = []
    ...
    # U-shape: alpha(1.5) < alpha(0.74) and alpha(1.5) < alpha(5.0)
    assert alphas[1] < alphas[0], "alpha should decrease from equilibrium"
    assert alphas[1] < alphas[2], "alpha should increase toward dissociation"
